        ...
        child: Column(
          children: [
            desktopPageHeader(
              context: context,
              title: 'التقارير',
              subtitle: 'لوحة إحصائية وتحليلية شاملة للبلاغات',
            ),
            Expanded(
              child: Obx(() {
                final reports = controller.allReports.toList();

                return desktopContent(
                  padding: const EdgeInsets.fromLTRB(28, 22, 28, 28),
                  child: reports.isEmpty && controller.isLoading.value
                      ? const Center(
...
class _ReportDashboard extends StatelessWidget {
  ...
  List<_CategoryRow> _rankedCategories() {
    ...
    final rows = map.entries.map((entry) {
      ...
      final solutionRate =
          stats.total == 0 ? 0.0 : stats.solved / stats.total * 100;
      final closedRate =
          stats.total == 0 ? 0.0 : closed / stats.total * 100;
  ...
  List<_MonthPoint> _lastSixMonths() {
    ...
    for (int offset = 5; offset >= 0; offset--) {
      ...
      points.add(
        _MonthPoint(
          label: '${monthDate.month.toString().padLeft(2, '0')}/${monthDate.year}',
          count: count,
        ),
      );
    }
  ...
  Widget build(BuildContext context) {
    ...
    final closedRate =
        total == 0 ? 0.0 : (solved + rejected) / total * 100;
    ...
    return pw.Container(
      ...
      decoration: pw.BoxDecoration(
        border: pw.Border.all(
          color: PdfColor.fromInt(0xFFDCE4E7),
        ),
          ...
          for (int i = 0; i < rows.length; i++)
            DataRow(
              cells: [
                ...
                DataCell(
                  Text('${rows[i].solutionRate.toStringAsFixed(1)}%'),
                ),
                DataCell(
                  Text('${rows[i].closedRate.toStringAsFixed(1)}%'),
                ),
              ],
            ),
        ],
